document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!product) {
    detailEl.innerHTML = `<div class="empty-state"><h2>找不到這項飲品</h2><p>可能已下架，請回到菜單重新選擇。</p><a class="btn btn-primary" href="index.html">回到菜單</a></div>`;
    return;
  }
  ...
  const gradeInfo = product.aroma
    ? `<ul class="notice-list" style="color:var(--primary-dark);">
        <li style="background:var(--bg-alt); border-color:var(--line); color:var(--text);">香氣：${product.aroma}｜口感：${product.taste}｜海拔 ${product.altitude}</li>
      </ul>`
    : '';
